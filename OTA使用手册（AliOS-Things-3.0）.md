...
相关功能介绍如下：
1. [ ]   OTA Secure Downloading Mode /*默认支持http下载，选中此项将支持https下载模式*/ 
2. [ ]   OTA via uAgent /*默认不支持uAgent模式升级，选中支持uAgent方式升级*/
3. [ ]   RSA Verify Support /*默认不支持安全升级，选中支持安全升级*/
4. (5)   OTA Download Retry Count /*默认网络异常重试5次，可以根据需要自定义次数*/
5. (20000) OTA Download Timeout(ms) /*OTA下载过程监控时间默认是20s,支持自定义*/
6. (512) OTA Download Block Size(bytes)/*OTA下载时获取数据buf大小默认是512字节，支持自定义*/
7. (1024) OTA write flash cache size(bytes)/*OTA写flash缓存大小默认是1k，支持自定义*/
8. [ ]   BLE OTA Support/*BLE OTA功能选项*/
完成配置后保存退出
